        ...
        try {
            ...
            if (is_array($resulttoupdate)) {
                if (count($resulttoupdate) > 0)
                    $societes = $this->eCommerceRemoteAccess->convertRemoteObjectIntoDolibarrSociete($resulttoupdate, $toNb);
            } else {
                $error++;
            }
...
//                            }
        ...
        try {
            ...
            if (is_array($resulttoupdate)) {
                ...
                if (count($resulttoupdate) > 0) {
                    ...
                    if (is_array($products) && count($products)) {
                        ...
                        if ($toNb <= 0)
                            dol_syslog('We get an empty array from convertRemoteObjectIntoDolibarrProduct() with input $resulttoupdate = ' . serialize($resulttoupdate), LOG_WARNING);
                        $error++;
                    }
                }
            }
            ...
            if (!$error && is_array($products)) {
                ...
                foreach ($products as $productArray) {
                        ...
                        if (!empty($ref)) {
                            ...
                            if ($refExists > 0) {
                                ...
                                if ($synchExists > 0 &&
                                    ...
                                    $error++;
                                    //$this->errors[] = 'product ID '.$dBProduct->id;
                    ...
                    if (!isset($dBProduct->stock_reel))
                        $dBProduct->stock_reel = 0;
                    ...
                    if ($dBProduct->id > 0) {
                        ...
                        if ($result >= 0) {
                            ...
                            if (isset($productArray['price_base_type']))
                                $price_base_type = $productArray['price_base_type'];
                        ...
                        if ($result >= 0) {
                            ...
                            $error++;
                            if ($dBProduct->error == 'ErrorProductAlreadyExists')
                                $this->error = $this->langs->trans('ECommerceSynchProductCreateError') . ' ' . $this->langs->trans($dBProduct->error, $dBProduct->ref);
                            else
                                $this->error = $this->langs->trans('ECommerceSynchProductCreateError') . ' ' . $dBProduct->error;
                            $this->errors[] = $this->error;
                            $this->errors = array_merge($this->errors, $dBProduct->errors);
                        }
                    ...
                    if (!$error && $result >= 0) {
                        ...
                        if (is_array($catsIds) && count($catsIds) > 0) {  // This product belongs at least to a category
                            ...
                            if (is_array($listofexistingcatsforproduct))
                                $listofexistingcatsforproduct = array_values($tmpcatids);
    ...
    public function synchCommande($toNb = 0) {
        ...
        try {
            ...
            if (is_array($resulttoupdate)) {
                if (count($resulttoupdate) > 0)
                    $commandes = $this->eCommerceRemoteAccess->convertRemoteObjectIntoDolibarrCommande($resulttoupdate, $toNb);
            } else {
                $error++;
            }
                        ...
                        if ($refExists > 0 && $dBCommande->id > 0) {  // Order already synch
                            ...
                            if (is_array($commandeArray['extrafields'])) {
                                ...
                                foreach ($commandeArray['extrafields'] as $extrafield => $extrafield_value) {
                                    if ($dBCommande->array_options['options_' . $extrafield] != $extrafield_value)
                                        $update_extrafields = true;
                            ...
                            if ($dBCommande->ref_client != $commandeArray['ref_client'] || $tmpdateorder1 != $tmpdateorder2 || $tmpdatedeliv1 != $tmpdatedeliv2
                            ) {
...
//        if (empty($mode) || preg_match('/^contacts/', $mode))
    ...
    function getContactIdFromInfos($contact) {
        ...
        if (isset($contact->country_id))
            $sql .= ' AND fk_pays=' . ($contact->country_id > 0 ? $contact->country_id : 'NULL');
        if (isset($contact->email))
            $sql .= ' AND email LIKE "' . $this->db->escape(trim($contact->email)) . '"';
        if (isset($contact->phone_pro))
            $sql .= ' AND phone LIKE "' . $this->db->escape(trim($contact->phone_pro)) . '"';
        if (isset($contact->fax))
            $sql .= ' AND fax LIKE "' . $this->db->escape(trim($contact->fax)) . '"';
