...
class eCommerceSynchro {
    ...
    function __construct($db, $site, $fromDate = null, $toDate = null, $toNb = 0) {
        ...
        try {
            ...
            if (empty($toDate))
                $this->toDate = (dol_now() - 10);      // Set date to use as last update date (we remove 10 second to be sure we don't have pb with not sync date)
            else
                $this->toDate = $toDate;
        } catch (Exception $e) {
    ...
    public function getProductToUpdate($force = false) {
        try {
            ...
            if (empty($this->productToUpdate) && (!empty($this->error) || !empty($this->errors) || !empty($this->eCommerceRemoteAccess->error) || !empty($this->eCommerceRemoteAccess->errors))) {
                if (!empty($this->eCommerceRemoteAccess->error))
                    $this->error = $this->eCommerceRemoteAccess->error;
                if (!empty($this->eCommerceRemoteAccess->errors))
                    $this->errors = array_merge($this->errors, $this->eCommerceRemoteAccess->errors);
                return -1;
            }
                ...
                if (!$error && is_array($categories)) {
                    ...
                    foreach ($categories as $categoryArray) {     // Loop on each categories found on ecommerce side. Cursor is $categoryArray
                        ...
                        if ($result >= 0) {
                            ...
                            if (!empty($categoryArray['updated_at']))
        ...
        try {
            ...
            if (!$contactExists) {
                ...
                if ($contactExists > 0) {
                    ...
                    if (isset($socpeopleArray['country_id']))
                        $dBContact->country_id = $socpeopleArray['country_id'];
                    if (isset($socpeopleArray['email']))
                        $dBContact->email = $socpeopleArray['email'];
                    if (isset($socpeopleArray['phone']))
                        $dBContact->phone_pro = dol_trunc($socpeopleArray['phone'], 30, 'right', 'UTF-8', 1);
                    if (isset($socpeopleArray['fax']))
                        $dBContact->fax = dol_trunc($socpeopleArray['fax'], 30, 'right', 'UTF-8', 1);
                    $dBContact->context['fromsyncofecommerceid'] = $this->eCommerceSite->id;
        ...
        try {
            ...
            if (is_array($resulttoupdate)) {
                if (count($resulttoupdate) > 0)
                    $factures = $this->eCommerceRemoteAccess->convertRemoteObjectIntoDolibarrFacture($resulttoupdate, $toNb);
            } else {
                $error++;
            }
...
//                $productsTypesOk = array('simple', 'virtual', 'downloadable');
    ...
    public function synchDtoECategory($toNb = 0) {
        ...
        foreach ($categories as $cat_id => $category) {
            if (in_array($cat_id, $already_synch))
                continue;
    ...
    public function dropImportedAndSyncData($deletealsoindolibarr, $mode = '') {
                    ...
                    if ($deletealsoindolibarr) {
                        ...
                        if ($dbFacture->fetch($this->eCommerceFacture->fk_facture) > 0) {
                            ...
                            if ((float) DOL_VERSION < 5.0)
                                ...
                                $resultdelete = $dbFacture->delete($this->user, 0, $idWarehouse);
        ...
        if (empty($mode) || preg_match('/^products/', $mode)) {
            ...
            if ($synchCategoriesDeleted)
                $this->success[] = $synchCategoriesDeleted . ' ' . $this->langs->trans('ECommerceResetCategoriesProductSuccess');
            unset($this->eCommerceProduct);
...
//        if (empty($mode) || preg_match('/^contacts/', $mode))
        ...
        if (empty($mode) || preg_match('/^thirdparties/', $mode)) {
            ...
            foreach ($arrayECommerceSocieteIds as $idSociete) {
                ...
                if ($this->eCommerceSociete->fetch($idSociete) > 0) {
                    ...
                    if ($this->eCommerceSociete->remote_id == 0)
                        continue;
        ...
        if (empty($mode) || preg_match('/^categories/', $mode)) {
            ...
            if ($deletealsoindolibarr)
                $this->success[] = $dolObjectsDeleted . ' ' . $this->langs->trans('ECommerceResetDolCategorySuccess') . ($dolObjectsNotDeleted ? ' (' . $dolObjectsNotDeleted . ' ko)' : '');
    ...
    function getContactIdFromInfos($contact) {
        ...
        if (isset($contact->country_id))
            $sql .= ' AND fk_pays ' . ($country_id > 0 ? '= ' . $country_id : ' IS NULL');
        if (isset($contact->email))
            $sql .= ' AND email LIKE "' . $this->db->escape($email) . '"';
        if (isset($contact->phone_pro))
            $sql .= ' AND phone LIKE "' . $this->db->escape($phone_pro) . '"';
        if (isset($contact->fax))
            $sql .= ' AND fax LIKE "' . $this->db->escape($fax) . '"';
        if ($socid > 0)
        ...
        try {
            ...
            if (is_array($companies_id)) {
                if (count($companies_id) > 0)
                    $societes = $this->eCommerceRemoteAccess->convertRemoteObjectIntoDolibarrSociete($companies_id, $toNb);
            } else {
                $error++;
            }
                ...
                foreach ($societes as $societeArray) {
                    ...
                    if ($synchExists > 0 && isset($this->eCommerceSociete->fk_societe)) {
                        ...
                        if ($refExists >= 0) {
                            ...
                            if (isset($societeArray['country_id']))
                                $dBSociete->country_id = $societeArray['country_id'];
                            if (isset($societeArray['default_lang']))
                                $dBSociete->default_lang = $societeArray['default_lang'];
...
//                            }
                ...
                if (empty($this->errors) && !$error) {
                    ...
                    if (!empty($nbgoodsunchronize))
                        $this->success[] = $nbgoodsunchronize . ' ' . $this->langs->trans('ECommerceSynchSocieteSuccess');
                    if (!empty($nbrecorderror))
                        $this->errors[] = $this->langs->trans('ECommerceSynchSocieteFailed', $nbrecorderror);
                    return -1;
                }
            } else {
    ...
    public function synchronizeCustomers($from_date = null, $to_date = null, $remote_ids = array(), $toNb = 0, $only_not_synchronized = false, $success_log = true) {
        ...
        if ($error) {
            $this->errors = array_merge(array($this->langs->trans('ECommerceErrorWhenSynchronizeCustomers')), $this->errors);
            if ($success_log && $nb_synchronized)
                $this->success[] = $this->langs->trans('ECommerceSynchronizeCustomersSuccess', $nb_synchronized);
            return -1;
        } else {
            if ($success_log)
                $this->success[] = $this->langs->trans('ECommerceSynchronizeCustomersSuccess', $nb_synchronized);
            ...
            try {
                ...
                if (!$error && $customer_data['remote_id'] >= 0 && !$bypass) {
                    ...
                    if (!$error && (!($third_party_id > 0) || $anonymous_customer)) {
                        ...
                        $result = $this->getThirdPartyByEmail($customer_data['email_key']); #MKDGS
                        if ($result < 0) {
                            $error++;
                        } else {
                    ...
                    if (!$error && $third_party_id > 0) {
                        ...
                        if ($result < 0) {
                            ...
                            if (!empty($third_party->error))
                                $this->errors[] = $third_party->error;
                    ...
                    if (!$error && (!$anonymous_customer || $third_party->id > 0)) {
                        ...
                        if (isset($customer_data['name_alias']))
                            $third_party->name_alias = $customer_data['name_alias'];
                        if (isset($customer_data['email']))
                            $third_party->email = $customer_data['email'];
                        ...
                        if (isset($customer_data['country_id']))
                            $third_party->country_id = $customer_data['country_id'];
                        if (isset($customer_data['default_lang']))
                            $third_party->default_lang = $customer_data['default_lang'];
                        ...
                        if (!$error) {
                            ...
                            if ($result < 0) {
                                if (!empty($third_party->error))
                                    $this->errors[] = $third_party->error;
                                $this->errors = array_merge($this->errors, $third_party->errors);
                                if (empty($third_party->error) && empty($third_party->errors))
    ...
    public function synchronizeProducts($from_date = null, $to_date = null, $remote_ids = array(), $toNb = 0, $success_log = true, $object_origin = null) {
        ...
        if ($error) {
            $this->errors = array_merge(array($this->langs->trans('ECommerceErrorWhenSynchronizeProducts')), $this->errors);
            if ($success_log && $nb_synchronized)
                $this->success[] = $this->langs->trans('ECommerceSynchronizeProductsSuccess', $nb_synchronized);
            return -1;
        } else {
            if ($success_log)
                $this->success[] = $this->langs->trans('ECommerceSynchronizeProductsSuccess', $nb_synchronized);
    ...
    public function synchronizeOrders($from_date = null, $to_date = null, $remote_ids = array(), $toNb = 0, $success_log = true) {
        ...
        if ($error) {
            $this->errors = array_merge(array($this->langs->trans('ECommerceErrorWhenSynchronizeOrders')), $this->errors);
            if ($success_log && $nb_synchronized)
                $this->success[] = $this->langs->trans('ECommerceSynchronizeOrdersSuccess', $nb_synchronized);
            return -1;
        } else {
            if ($success_log)
                $this->success[] = $this->langs->trans('ECommerceSynchronizeOrdersSuccess', $nb_synchronized);
                                ...
                                if (!$error) {
                                    ...
                                    if ($result < 0) {
                                        if (!empty($order->error))
                                            $this->errors[] = $order->error;
                                        $this->errors = array_merge($this->errors, $order->errors);
                                        if (empty($order->error) && empty($order->errors))
                                            $this->errors[] = $this->db->lasterror();
                                        $error++;
                                    }
            ...
            try {
                ...
                if ($result < 0) {
                    ...
                    if (!$error) {
                        ...
                        if ($invoice->id > 0) {
                            if ($order->id > 0)
                                $invoice->add_object_linked($order->element, $order->id);
                                    ...
                                    if (!$error) {
                                        if ($order->id > 0) {
                                            ...
                                            if (empty($order->lines)) {
                                                if (method_exists($order, 'fetch_lines'))
                                                    $order->fetch_lines();
                                                elseif (method_exists($order, 'getLinesArray'))
                                                    $order->getLinesArray();
                                            }
...
//									}
                                    ...
                                    if (!$error && !empty($this->eCommerceSite->parameters['order_actions']['create_supplier_invoice']) && is_array($order_data['fee_lines']) && count($order_data['fee_lines']) > 0) {
                                        ...
                                        if ($supplier_id == 0) {
                                            ...
                                            if ($id < 0) {
                                                ...
                                                if (!empty($supplier_invoice->error))
                                                    $this->errors[] = $supplier_invoice->error;
                                            ...
                                            if (!$error) {
                                                ...
                                                foreach ($order_data['fee_lines'] as $fee_line) {
                                                    if (floatval(DOL_VERSION) < 8)
                                                        $this->db->begin(); // Not exist in addline function but commit and rollback exist
                                            ...
                                            if (!$error && !empty($conf->global->INVOICE_SUPPLIER_ADDON_PDF) && empty($conf->global->MAIN_DISABLE_PDF_AUTOUPDATE)) {
                                                ...
                                                $newlang = '';
                                                if ($conf->global->MAIN_MULTILANGS && empty($newlang))
                                                    $newlang = $supplier_invoice->thirdparty->default_lang;
                                                if (!empty($newlang)) {
                                                    $outputlangs = new Translate("", $conf);
                                                    $outputlangs->setDefaultLang($newlang);
                                                    $outputlangs->load('products');
                                    ...
                                    if (!$error && !empty($this->eCommerceSite->parameters['order_actions']['send_invoice_by_mail'])) {
                                        ...
                                        if (!$error) {
                                            ...
                                            if (!empty($conf->dolimail->enabled))
                                                $this->langs->load("dolimail@dolimail");
                                            ...
                                            if (is_numeric($arraydefaultmessage) && $arraydefaultmessage < 0) {
                                                ...
                                                if (!empty($formmail->error))
                                                    $this->errors[] = $formmail->error;
                                                else
                                                    $this->errors[] = $this->db->lasterror();
                                                $error++;
                                            }
                                                    ...
                                                    if ($result) {
                                                        ...
                                                        if (!is_array($contact_list)) {
                                                            ...
                                                            if ($result < 0) {
                                                                ...
                                                                if (!empty($interface->error))
                                                                    $this->errors[] = $interface->error;
                            ...
                            if (!$error && $conf->global->$last_sync_date < $sync_date) {
                                $result = dolibarr_set_const($this->db, $last_sync_date, $sync_date, 'chaine', 0, '', $conf->entity);
                                if ($result < 0) {
                                    $this->errors[] = $this->langs->trans('ECommerceErrorSetLastSyncDateOrder');
                                    $this->errors[] = $this->db->lasterror();
    ...
    public function getThirdPartyByEmail($email, $site_id = 0) {
        ...
        if ($site_id > 0)
            $sql .= " LEFT JOIN " . MAIN_DB_PREFIX . "ecommerce_societe AS es ON es.fk_societe = s.rowid";
        $sql .= " WHERE (s.email = '$email' OR sp.email = '$email')";
        if ($site_id > 0)
            $sql .= " AND es.fk_site = $site_id";
        $sql .= " AND s.status = 1";
    ...
    public function createThirdParty($order_customer_ref, $company, $firstname, $lastname, $address, $zip, $town, $country_id, $email, $phone, $fax) {
        ...
        if (!empty($order_customer_ref))
            $third_party->update_note("Site: '{$this->eCommerceSite->name}' - Order: {$order_customer_ref}", '_private');
    ...
    public function addLineToObject($object, $desc, $pu_ht, $qty, $txtva, $total_ht, $total_tva, $total_ttc,
        ...
        if ($object->statut == $object::STATUS_DRAFT) {
            ...
            if (empty($txlocaltax2))
                $txlocaltax2 = 0;
            if (empty($total_localtax1))
                $total_localtax1 = 0;
            if (empty($total_localtax2))
                $total_localtax2 = 0;
            if (empty($fk_parent_line) || $fk_parent_line < 0)
                $fk_parent_line = 0;
            if (empty($object->fk_multicurrency))
                $object->fk_multicurrency = 0;
            if (empty($fk_prev_id))
                $fk_prev_id = 'null';
    ...
    public function loadPaymentGateways() {
        if (!isset(self::$payment_gateways_cached)) {
            ...
            if (!is_array($result)) {
                ...
                if (!empty($pay_gateways->error))
                    $this->errors[] = $pay_gateways->error;
