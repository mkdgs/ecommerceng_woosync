...
class eCommerceRemoteAccessWoocommerce {
    ...
    public function getSocieteToUpdate($fromDate, $toDate) {
        ...
        while (true) {
            ...
            if (!is_array($page) || ($nbCustomers = count($page)) == 0)
                break;
    ...
    public function getProductToUpdate($fromDate, $toDate) {
        ...
        while (true) {
            try {
                $page = $this->client->get('products', $filter);
            } catch (HttpClientException $fault) {
            ...
            if (!isset($page->products) || ($nbProducts = count($page->products)) == 0)
                break;
            $page = $page->products;
            ...
            foreach ($page as $product) {
                $id = $product->id;
                $update = false;
                $date_product = $this->getDateTimeFromGMTDateTime(!empty($product->date_modified) ? $product->date_modified : $product->date_created);
                $update_parent = false;
    ...
    public function getCommandeToUpdate($fromDate, $toDate) {
        ...
        while (true) {
            ...
            if (!isset($page->orders) || ($nbOrders = count($page->orders)) == 0)
                break;
            $page = $page->orders;
    ...
    public function convertRemoteObjectIntoDolibarrSociete($from_date = null, $to_date = null, $remoteObject = array(), $toNb = 0) {
        ...
        if ($toNb > 0)
            $filters['per_page'] = min($nb_max_by_request, $toNb);
        if (!empty($remoteObject))
            $filters['include'] = implode(',', $remoteObject);
        else {
            if (isset($from_date))
                $filters['after'] = $from_date;
            if (isset($to_date))
                $filters['before'] = $to_date;
        }
    ...
    public function convertRemoteObjectIntoDolibarrProduct($from_date = null, $to_date = null, $remoteObject = array(), $toNb = 0) {
        ...
        foreach ($remoteObject as $id) {
            ...
            if (isset($ids[1])) {
                if (!isset($include_variation_ids[$remote_id]))
                    $include_variation_ids[$remote_id] = array();
 ...
 */
    ...
    public function convertProductDataIntoProcessedData($remote_data, $parent_remote_data = null) {
        ...
        if (!$isVariation)
            $product['tax_rate'] = $tax_info['tax_rate'];
 ...
 */
    ...
    public function convertOrderDataIntoProcessedData($remote_data) {
        ...
        foreach ($remote_data->line_items as $item) {
            ...
            if (isset($item->cog_item_cost))
                $item_data['buy_price'] = $this->site->ecommerce_price_type == 'TTC' ? 100 * $item->cog_item_cost / (100 + $item_data['tva_tx']) : $item->cog_item_cost;
            if ($this->site->ecommerce_price_type == 'TTC')
                $item_data['price'] = (100 * ($item->subtotal + $item->subtotal_tax) / (100 + $item_data['tva_tx'])) / $item->quantity;
        ...
        if (!empty($remote_data->shipping_lines)) {
            ...
            foreach ($remote_data->shipping_lines as $item) {
                ...
                if ($this->site->ecommerce_price_type == 'TTC')
                    $item_data['price'] = 100 * ($item->total + $item->total_tax) / (100 + $item_data['tva_tx']);
            ...
            foreach ($remote_data->coupon_lines as $item) {
                ...
                foreach ($remote_data->tax_lines as $data) {
                    if (empty($data->tax_total))
                        continue;
                    if ($data->rate_percent > $tax_rate)
                        $tax_rate = $data->rate_percent;
                }
...
//			$status = Commande::STATUS_DRAFT;   // draft by default
        ...
        $order = [
            ...
            'fee_lines' => $fee_lines,
            'extrafields' => [
                "ecommerceng_online_payment_site_id_{$conf->entity}" => $this->site->id,
                "ecommerceng_online_payment_method_{$conf->entity}" => $remote_data->payment_method,
        ...
        if ($isProductVariation) { // Variations
            ...
            if (!empty($object->array_options)) {
                foreach ($object->array_options as $key => $value) {
                    $cr_key = substr($key, 8);
                    if (preg_match('/^ecommerceng_/', $cr_key))
                        continue;
                    $options_saved = $this->site->parameters['ef_crp']['product'][$cr_key];
                    if ($options_saved['activated']) {
                        $rm_key = $cr_key;
                        if (isset($options_saved['correspondences']))
                            $rm_key = $options_saved['correspondences'];
    ...
    public function updateRemoteCommande($remote_id, $object) {
        ...
        if (!empty($status)) {
            ...
            if ($order_status[$status] < $order_status[$wc_status] &&
                    !empty($conf->global->ECOMMERCENG_WOOCOMMERCE_ORDER_STATUS_LVL_CHECK))
                $status = $wc_status;
